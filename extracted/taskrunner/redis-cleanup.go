...
var (
	...
	CleanupRunning   bool
	Campaigns        []Campaign_Row_Clean
	Exceptions       []string
	CampaignFileUids []string
	CampaignFilePath string
)

func RedisCleanup() {
	fmt.Printf("Received request for redis cleanup!\n")
	if CleanupRunning == false {
		fmt.Printf("Redis cleanup is running..\n")
		CleanupRunning = true
		Exceptions = []string{"cache", ""}
		fmt.Printf("Redis cleaner Version %s (%s) Build %s\n", Version, Commit, Build)
		GetCampaigns()
		ReadCampaignLogFiles()
		CleanTrackingLogs()
		//WalkRedisCampaigns()
		//        CleanOldTrackings() // should be enabled on most normal deployments
		CleanupRunning = false
		fmt.Printf("Redis cleanup is finished!\n")
	} else {
		fmt.Printf("Redis cleanup is already running..\n")
	}
}

func ReadCampaignLogFiles() {
	fmt.Printf("Reading files...\n")
	Pathas, err := filepath.Abs(filepath.Dir(os.Args[0]))
	if err != nil {
		ProgramPanic(err)
	}
	fmt.Printf("Taskrunner path detected as: %s\n", Pathas)
	CampaignFilePath = Pathas + "/../campaign_logs"
	fmt.Printf("Campaign logs directory: %s\n", CampaignFilePath)
	files, err := ioutil.ReadDir(CampaignFilePath)
	if err != nil {
		ProgramPanic(err)
	}
	for _, file := range files {
		if !file.IsDir() {
			CampaignFileUids = append(CampaignFileUids, file.Name())
		}
	}
	fmt.Printf("We got filenames from camapign logs: %#v\n", CampaignFileUids)
}

func DeleteByWildcard(wildcard string) {
	var cursor uint64
	for {
		var keys []string
		var err error
		keys, cursor, err = redisdb.Scan(cursor, "*"+wildcard+"*", 0).Result()
		if err != nil {
			panic(err)
		}
		for _, key := range keys {
			fmt.Println("deleting key from redis: ", key)
			redisdb.Del(key)
		}
...
func DeleteTrackings(uid string) {
	fmt.Printf("Deleting tracking logs from campaign uid: %s\n", uid)
	counter := 0
	FilePath := ProgramPath + "/../campaign_logs/" + uid
	...
	for scanner.Scan() {
		counter++
		line := scanner.Text()
		err := redisdb.HDel("tracking_logs", line).Err()
		if err != nil {
			fmt.Printf("error deleting from redis: %s\n", err)
		}
		fmt.Printf("Deleting from line: (%d) %s\r", counter, line)
	}
	fmt.Printf("Deleting tracking logs have finished for campaign: %s deleted records count: %d\n", uid, counter)
	if counter > 0 {
		fmt.Printf("Deleting campaign logs file: %s\n", FilePath)
		e := os.Remove(FilePath)
		if e != nil {
			fmt.Printf("unable to remove the file: %s err: %s\n", FilePath, e)
		}
	}

}

func CleanTrackingLogs() {
	for _, uid := range CampaignFileUids {
		if !DoExistCampaignInMysql(uid) {
			fmt.Printf("Campaign: %s seems like deleted already\n", uid)
			DeleteByWildcard(uid)
			DeleteTrackings(uid)
		}
	}
...
type Campaign_Row_Clean struct {
	Id         int      `db:"id"`
	Uid        string   `db:"uid"`
	Name       string   `db:"name"`
...
func ReturnCampainObj(uid string) Campaign_Row_Clean {
	returnas := Campaign_Row_Clean{}
	for _, row := range Campaigns {
		if row.Uid == uid {
			returnas = row
		}
