...
my $rand_file = "send_adresses.txt";
...
sub rand_nr {
my $rand1 = int(rand(300));
my $rand2 = int(rand(100000))+20;
return "$rand1$rand2";
}
...
my $stha = $dbh->prepare("UPDATE campaigns set from_email = ?, reply_to = ?, subject = ?, from_name = ?");
$stha->execute($reply,$reply,"Du hast eine neue Nachricht erhalten! ".rand_nr(),"Vanessa Via M".rand_nr());
...
set_mysql($randomline,"noreply\@".$randomline,$mail_list_id); 
