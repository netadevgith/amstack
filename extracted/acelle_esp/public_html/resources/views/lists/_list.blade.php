		...
		@foreach ($lists as $key => $item)
			<tr>
				...
				<td>
					...
					@else
						{{--@foreach ($item->listActiveServers() as $key2 => $item2)--}}
						{{--<div class="text-danger"><i class="icon-info3"></i> {{ $item2 }}</div>--}}
						{{--@endforeach--}}
					@endif
				</td>
				...
				<td>
					<div class="dropdown" style="display:none">
						<button class="dropbtn">Server quick select</button>
						<div class="dropdown-content">
					@foreach ($item->selectableServers($item->id) as $keyb )
						@if ($keyb->sending_server_id > 0)
							<b><a href="servers/setserver/{{ $item->id }}/{{ $keyb->id }}">{{ $keyb->name }}</a></b>
						@else
									<a href="servers/setserver/{{ $item->id }}/{{ $keyb->id }}">{{ $keyb->name }}</a>
						@endif
					@endforeach
						</div>
					</div>
...
{{--							<li><a href="{{ action('SegmentController@index', $item->uid) }}"><i class="icon-make-group"></i> {{ trans('messages.segments') }}</a></li>--}}
							...
							{{--<li>--}}
								{{--<a class="level-1" href="{{ action('FieldController@index', $item->uid) }}">--}}
									{{--<i class="icon-list3"></i> {{ trans('messages.manage_list_ƒds') }}--}}
								{{--</a>--}}
							{{--</li>--}}
							...
							@endif
							<li><a href="{{ action('SubscriberController@recover', $item->uid) }}"><i class="icon-minus-circle2"></i>Recover Unsubscribers</a></li>
							<li><a href="{{ action('SubscriberController@remove', $item->uid) }}"><i class="icon-cancel-circle2"></i>Remove Contacts</a></li>
							<li>
