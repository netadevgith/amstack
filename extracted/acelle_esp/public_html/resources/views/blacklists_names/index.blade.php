...
@section('page_header')
    <div class="page-title">
        ...
        <h1>
            <span class="text-semibold"><i class="icon-list2"></i> Names blacklist</span>
        </h1>
        <h3>
...
@section('content')
    ...
    <form class="listing-form"
        data-url="{{ action('Blacklist_namesController@listing') }}"
        per-page="{{ Acelle\Model\Blacklist_names::$itemsPerPage }}"
    >
                    ...
                    <div class="filter-box">
                        ...
                        <div class="btn-group list_actions hide">
                            ...
                            <ul class="dropdown-menu">
                                <li><a delete-confirm="{{ trans('messages.remove_blacklist_confirm') }}" href="{{ action('Blacklist_namesController@delete') }}"><i class="icon-trash"></i> {{ trans('messages.delete') }}</a></li>
                            </ul>
                        </div>
                        <span class="filter-group">
                            <span class="title text-semibold text-muted">{{ trans('messages.sort_by') }}</span>
                            <select class="select" name="sort-order">
                                <option value="blacklist_names.created_at">{{ trans('messages.created_at') }}</option>
                                <option value="blacklist_names.name">Names</option>
                            </select>
            ...
            <div class="col-md-3">
                {{--@if (Auth::user()->customer->can('import', new Acelle\Model\Blacklist()))--}}
                    <div class="text-right">
                        <a href="{{ action('Blacklist_namesController@import') }}" type="button" class="btn bg-info-800">
                            <i class="icon-download4"></i> {{ trans('messages.blacklist.import') }}
                        </a>
                    </div>
                {{--@endif--}}
                    <div class="text-right">
                        <a href="{{ action('Blacklist_namesController@item_add') }}" type="button" class="btn bg-info-800">
