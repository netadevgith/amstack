        ...
        <form action="{{ action('CampaignController@confirm', $campaign->uid) }}" method="POST" class="form-validate-jqueryz">
            ...
            @if ($campaign->step() >= 5)
                ...
                <div class="text-right">
                    ...
                    <button class="btn btn-lg bg-grey mr-5 send-a-test-mass-link" data-uid="{{ $campaign->uid }}">Send a test email from all servers <i class="icon-envelop4 ml-5"></i> </button>
                    <button class="btn btn-lg bg-grey mr-5 send-a-test-domain-link" data-uid="{{ $campaign->uid }}">Send a test email from all domains <i class="icon-envelop4 ml-5"></i> </button>
                    <button id="start_simulation" class="btn btn-lg bg-grey mr-5 send-simulation" data-uid="{{ $campaign->uid }}">Simulation test <i class="icon-gear ml-5"></i> </button>
                    <button style="display:none" id="stop_simulation" class="btn btn-lg bg-teal-800 stop-simulation" data-uid="{{ $campaign->uid }}">Stop simulation</button>
                    <br />
                    <br />
                    <button class="btn btn-lg bg-teal-800">{{ trans('messages.send') }} <i class="icon-paperplane ml-5"></i> </button>
                </div>
            <div style="display:none" id="display_debug">
                <br>
                <p>Campaign simulation status:</p>
                <textarea rows="15" cols="100" id="debug_status" name="debug_status"></textarea>
            </div>
            @endif
        </form>
...
<script>
    ...
    function stop_simulation(uidas) {

    }

    function refresh_simulation_status() {
        var uid = '{{ $campaign->uid }}';
        var i = setInterval(function(){
            jQuery.ajax({
                type:"GET",
                url:"/campaigns/simulation/{{ $campaign->uid }}",
                dataType:"json",
                success:function(data) {
                    jQuery('#debug_status').val(data.status_raw);
                    $('#debug_status').scrollTop($('#debug_status')[0].scrollHeight);
                }
            });
        },1000);
    }
