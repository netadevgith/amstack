	...
	>
		...
		@foreach ($blacklists as $key => $blacklist)
			<tr>
				...
				<td>
					<span class="no-margin kq_search">{{ $blacklist->domain }}</span>
					<span class="text-muted second-line-mobile">{{ trans('messages.email') }}</span>
				</td>
				<td>
					<span class="no-margin kq_search">{{ $blacklist->reason }}</span>
					<span class="text-muted second-line-mobile">Comment</span>
				</td>
				<td>
				...
				<td class="text-right">
					{{--@if (Auth::user()->customer->can('delete', $blacklist))--}}
						<a
							...
							href="{{ action('Blacklist_domainsController@delete', ["uids" => $blacklist->id]) }}"
