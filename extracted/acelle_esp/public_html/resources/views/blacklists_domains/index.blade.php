...
@section('page_header')
    <div class="page-title">
        ...
        <h1>
            <span class="text-semibold"><i class="icon-list2"></i> Domains blacklist</span>
        </h1>
        <h3>
            @include("blacklists._links")
        </h3>
    </div>
@endsection
...
@section('content')
    ...
    <form class="listing-form"
        data-url="{{ action('Blacklist_domainsController@listing') }}"
        per-page="{{ Acelle\Model\Blacklist_domains::$itemsPerPage }}"
    >
                    ...
                    <div class="filter-box">
                        ...
                        <div class="btn-group list_actions hide">
                            ...
                            <ul class="dropdown-menu">
                                <li><a delete-confirm="{{ trans('messages.remove_blacklist_confirm') }}" href="{{ action('Blacklist_domainsController@delete') }}"><i class="icon-trash"></i> {{ trans('messages.delete') }}</a></li>
                            </ul>
                        </div>
                        <span class="filter-group">
                            <span class="title text-semibold text-muted">{{ trans('messages.sort_by') }}</span>
                            <select class="select" name="sort-order">
                                <option value="blacklist_domains.created_at">{{ trans('messages.created_at') }}</option>
                                <option value="blacklist_domains.domain">Domains</option>
                            </select>
            ...
            <div class="col-md-3">
                {{--@if (Auth::user()->customer->can('import', new Acelle\Model\Blacklist()))--}}
                    <div class="text-right">
                        <a href="{{ action('Blacklist_domainsController@import') }}" type="button" class="btn bg-info-800">
                            <i class="icon-download4"></i> {{ trans('messages.blacklist.import') }}
                        </a>
                    </div>
                {{--@endif--}}
                    <div class="text-right">
                        <a href="{{ action('Blacklist_domainsController@item_add') }}" type="button" class="btn bg-info-800">
                            <i class="icon-add"></i> Add contact
                        </a>
                        {{--<a href="{{ action('Blacklist_domainsController@deleteall') }}" type="button" class="btn bg-info-800">--}}
