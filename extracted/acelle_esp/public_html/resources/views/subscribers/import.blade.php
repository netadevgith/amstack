	...
	<div class="row">
		<div class="col-md-12">
			<form
				...
				<h3>Duplicate check on the following mail lists (Contacts will not be imported if already exists in these lists):</h3>
				<select id="maillistai" name="listai[]" class="multiselect" multiple="multiple">
                @foreach ($listai as $maillistas)
						<option value="{{ $maillistas->id }}">{{ $maillistas->name }}</option>
					@endforeach
				</select>
				<script>
