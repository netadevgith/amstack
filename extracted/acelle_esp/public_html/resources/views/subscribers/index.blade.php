...
@section('content')
				...
				>
					<div class="row top-list-controls">
							...
							@if ($list->readCache('SubscriberCount', 0) > 0)
								<div class="filter-box">
									...
									<span class="text-nowrap">
										<input name="keyword" class="form-control search" placeholder="{{ trans('messages.type_to_search') }}" id="searchas" />
                                        <i class="icon-search4 keyword_search_button"></i><a title="Export results" onclick="exportsearchcriteria({{ $list->id }})" type="button" class="btn bg-info-800"><i class="icon icon-database-export"></i></a>
							</a>
									</span>
								</div>
							@endif
						...
						<div class="col-md-2 text-right" style="display:none">
							<a href="{{ action("SubscriberController@delete_dupes", $list->uid) }}" type="button" class="btn bg-info-800">
								<i class="icon icon-minus2"></i> Clean duplicates
							</a>
						</div>
					</div>
	...
	<script>
		function exportsearchcriteria(idas) {
		    var criteria
		    criteria = $('#searchas').val()
            if (criteria.length > 1) {
                window.open('/list/searchingexport/' + idas + '/' + criteria);
            } else {
		        alert('Your search keyword is empty!');
            }
