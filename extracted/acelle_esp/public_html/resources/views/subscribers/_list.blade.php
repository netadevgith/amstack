			...
			<tr>
				...
				<td>
					<div class="no-margin text-bold">
						...
						@elseif ($item->status == "unsubscribed" && $item->getunsubscribecounts($item->id) > 0)
							<span class="label label-flat bg-{{ $item->status }}">Unsubscribed</span> Count: {{ $item->getunsubscribecounts($item->id) }}</span>
						@else
				...
				@if (in_array("updated_at", explode(",", request()->columns)))
					<td>
						...
						<br>
						<span class="text-muted2">{{ trans('messages.opened_at') }}</span>
							@else
					...
					<div class="btn-group">
						...
						<ul class="dropdown-menu dropdown-menu-right">
							...
							@endif
								...
								<li><a class="ajax_link" href="{{ action('BlacklistController@item_add', ['email' => $item->email, "comment" => 'Manaul add to supression list']) }}"><i class="icon-exit2"></i> Move to blacklist</a></li>
