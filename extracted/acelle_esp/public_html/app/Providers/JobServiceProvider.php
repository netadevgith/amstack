...
class JobServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        \Queue::before(function (\Illuminate\Queue\Events\JobProcessing $event) {
            MailLog::info($event->job->resolveName());
        ...
        \Queue::after(function (\Illuminate\Queue\Events\JobProcessed $event) {
            //MailLog::notice($event->job->resolveName());
            $job = $this->getJobObject($event);
            $job->setDone();
            //return true;
        });
        ...
        \Queue::failing(function (\Illuminate\Queue\Events\JobFailed $event) {
            //MailLog::error($event->job->resolveName());
            $job = $this->getJobObject($event);
            $job->setFailed();
            //return true;
        });
        MailLog::info("JobServiceProvider.php:boot:49 Seems to be finished!");
        //return true;
    }
    ...
    private function getJobObject($event) {
        ...
        return unserialize($jsonpayload['data']['command']);
    }
