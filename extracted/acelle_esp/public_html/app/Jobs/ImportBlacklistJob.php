...
class ImportBlacklistJob extends ImportExportJob
{
     ...
     * @return void
     */
    public function __construct($file, $comment, $customer=NULL, $admin=NULL)
    {
    ...
    public function handle()
    {
        ...
        if ($status != "running") {
            MailLog::info("Starting first blacklist import process");
            \Acelle\Model\Blacklist::import($this->file, $this->comment, $this, $this->customer, $this->admin);
        } else {
            MailLog::info("Blacklist job is already running!");
        }
    }
