...
class ImportSubscribersJob extends ImportExportJob
{
     ...
     * @return void
     */
    public function __construct($mailList, $customer, $file, $exceptions)
    {
        // call parent's constructor
        parent::__construct($mailList, $customer, NULL, $exceptions);
     //   MailLog::info("is _construct". print_r($exceptions,true));
        // Upload csv
    ...
    {
     //   MailLog::info("is handle: ".print_r($this->exceptions,true));
        $status =json_decode($this->getSystemJob()->data)->status;
        if ($status != "running" and $status != "done" and $status != "cancelled" and $status != "failed") {
            // change it to 2 for the old method
            // change it to 4 for my latest method
        $this->mailList->import2($this->file, $this->customer, $this, $this->exceptions);
        }  else {
            MailLog::info("Import job is already running!");
        }
    }
