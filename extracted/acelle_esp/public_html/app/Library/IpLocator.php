<?php
/* IpLocator class (c) 2018-2019 justinas@eofnet.lt
*/
...
class IpLocator
{
    ...
    private function geoip_load_shared_mem($file)
    {
        $fp = fopen($file, "rb");
        if (!$fp) {
        ...
        $s_array = fstat($fp);
        $size = $s_array['size'];
        if (($shmid = @shmop_open(0x4f415401, "w", 0, 0))) {
            shmop_delete($shmid);
            shmop_close($shmid);
        }
        $shmid = shmop_open(0x4f415401, "c", 0644, $size);
        ...
        if ($gi->flags & 2) {
            ...
            for ($i = 0; $i < 20; $i++) {
                ...
                if ($delim == (chr(255) . chr(255) . chr(255))) {
                    ...
                    if ($gi->databaseType == 7) {
                        $gi->databaseSegments = 16700000;
                    } elseif ($gi->databaseType == 3) {
                        $gi->databaseSegments = 16000000;
                    } elseif (($gi->databaseType == 6)
                        || ($gi->databaseType == 2)
                        || ($gi->databaseType == 5)
                        || ($gi->databaseType == 23)
                        || ($gi->databaseType == 11)
...
# $start + $maxLength may be greater than the shared mem size
    private function _sharedMemRead($gi, $start, $maxLength)
    {
    ...
    public function geoip_open($filename, $flags)
    {
        ...
        $gi->flags = $flags;
        if ($gi->flags & 2) {
            $gi->shmid = shmop_open(0x4f415401, "a", 0, 0);
        } else {
    ...
    public function geoip_country_code_by_name_v2($gi, $name)
    {
    ...
    public function geoip_country_name_by_name_v2($gi, $name)
    {
        $country_id = $this->geoip_country_id_by_name($gi, $name);
        if ($country_id !== false) {
            return $gi->GEOIP_COUNTRY_NAMES[$country_id];
    ...
    public function geoip_country_id_by_addr_v6($gi, $addr)
    {
        ...
        return $gi->_geoip_seek_country_v6($gi, $ipnum) - 16776960;
    }

    public function geoip_country_id_by_addr($gi, $addr)
    {
        $ipnum = ip2long($addr);
        return $this->_geoip_seek_country($gi, $ipnum) - 16776960;
    }
    ...
    private function _common_get_org($gi, $seek_org)
    {
        $record_pointer = $seek_org + (2 * $gi->record_length - 1) * $gi->databaseSegments;
        if ($gi->flags & 2) {
            $org_buf = $this->_sharedMemRead($gi, $record_pointer, 300);
        } else {
            fseek($gi->filehandle, $record_pointer, SEEK_SET);
            $org_buf = fread($gi->filehandle, 300);
        }
        $org_buf = $this->_safe_substr($org_buf, 0, strpos($org_buf, "\0"));
        return $org_buf;
    }

    private function _get_org_v6($gi, $ipnum)
    {
    ...
    private function _get_org($gi, $ipnum)
    {
    ...
    private function geoip_name_by_addr_v6($gi, $addr)
    {
        if ($addr == null) {
            return 0;
        }
    ...
    private function geoip_name_by_addr($gi, $addr)
    {
        if ($addr == null) {
            return 0;
        }
    ...
    private function geoip_org_by_addr($gi, $addr)
    {
    ...
    {
        if ($gi->databaseType == 7) {
            $seek_region = $this->_geoip_seek_country($gi, $ipnum) - 16700000;
            if ($seek_region >= 1000) {
                $country_code = "US";
            ...
            $seek_region = $this->_geoip_seek_country($gi, $ipnum) - 16000000;
            if ($seek_region < 1) {
                $country_code = "";
                $region = "";
            } elseif ($seek_region < 677) {
                $country_code = "US";
                $region = chr(($seek_region - 1) / 26 + 65) . chr(($seek_region - 1) % 26 + 65);
            } elseif ($seek_region < 1353) {
                $country_code = "CA";
                $region = chr(($seek_region - 677) / 26 + 65) . chr(($seek_region - 677) % 26 + 65);
            } else {
                $country_code = $gi->GEOIP_COUNTRY_CODES[(int)(($seek_region - 1353) / 360)];
                $region = "";
            }
            return array($country_code, $region);
        }
    ...
    private function geoip_region_by_addr($gi, $addr)
    {
        if ($addr == null) {
            return 0;
        }
