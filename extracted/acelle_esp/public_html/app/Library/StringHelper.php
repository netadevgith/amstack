...
class StringHelper
{
    ...
    public static function base64UrlEncode2($val)
    {
        //return strtr(base64_encode($val), '+/=', '-_,');
        return rtrim(strtr(base64_encode($val), '+/', '-_'), '=');
    }

    public static function base64UrlEncode3($val) {

        return strtr(base64_encode(gzdeflate($val)),'+/=', '-_,');
    }
    ...
    public static function base64UrlDecode2($val)
    {
        return base64_decode(str_pad(strtr($val, '-_', '+/'), strlen($val) % 4, '=', STR_PAD_RIGHT));
    }

    public static function base64UrlDecode3($val) {
        return gzinflate(base64_decode(strtr($val,'-_,', '+/=')));
    }
    ...
    public static function generateCampaignName($base = null)
    {
        $rand = rand(10000, 9999999).'c0a0m0p'.rand(9999,3000000);
        return $rand;
    }

    public static function generateUnsubscribeUrl($base = null)
    {
        $rand = rand(10000, 9999999).'b0d03v9'.rand(9999,3000000);
        return $rand;

    }

    public static function genearateclickUrl($base = null)
    {
        $rand = rand(10, 999).'d2g8t0'.rand(99,300000);
        return $rand;
