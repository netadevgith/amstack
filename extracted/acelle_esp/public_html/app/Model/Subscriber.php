...
class Subscriber extends Model
{
    ...
    public function getunsubscribecounts($uid) {
     //   \DB::enableQueryLog();
     //   MailLog::info("mail id: ".$uid);
      $la = \DB::table('subscribers')
      ->select('subscribers.email')
      ->leftjoin('tracking_logs','subscribers.id','=','tracking_logs.subscriber_id')
      ->leftjoin('unsubscribe_logs','tracking_logs.message_id','=','unsubscribe_logs.message_id')
      ->where('subscribers.id','=',$uid)->count();
    //  MailLog::info("countas: ".print_r($la,true));
    //  MailLog::info('log'.print_r(\DB::getQueryLog(),true));
        return $la;
    }
