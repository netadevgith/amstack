...
class SendingServer extends Model
{
    ...
    protected $fillable = [
        ...
        'bounce_handler_id', 'feedback_loop_handler_id', 'status','threads'
    ];
    ...
    public static function filter($request)
    {
        ...
        if (!empty($filters)) {
            ...
            if (!empty($filters['sort-order2']))
                $query = $query->where('sending_servers.servgroup','=',$filters['sort-order2']);
        }
    ...
    public static function search($request)
    {
        ...
        if(!empty($request->sort_order)) {
            $query = $query->where('id' ,'>','1')->orderBy($request->sort_order, $request->sort_direction);
        }
    ...
    {
        //MailLog::info('TEST MAIL FROM SENDING SERVER MODEL FUNCTION USED');
        /**
        ...
        if ($this->type == "smtp") {
            ...
            //$headers = array('Osver' => 'winme');
            $send_data = array('server_ip' => $this->host, 'port' => (int)$this->smtp_port, 'from_email' => $params['from_email'],'to_email' =>$params['to_email'], 'subject' => $params['from_email'], 'body' => $params['plain']);
            ...
            MailLog::info("EXPERIMENTAL smtp send issued");
        } else {
            MailLog::info("Type: " . $this->type);
            MailLog::info("debug: " . print_r($this, true));
            $message = \Swift_Message::newInstance();
            $msgId = StringHelper::generateMessageId(StringHelper::getDomainFromEmail($params['from_email']));
            $message->setId($msgId);
// headeriu pridejimas
            $message->getHeaders()->addTextHeader('X-My-Message-Id', $msgId); // this header is required for SendGrid API sending server
            ...
            //MailLog::info('Ok part2');
            $result = self::getInstance($this)->send($message);
            //MailLog::info('Ok part3');
    ...
    public function initializeSns($params)
    {
        $result = self::getInstance($this)->setupSns($params);
        return true;
    }
