...
class Customer extends Model
{
    ...
    public function campaigns()
    {
        ...
        return $this->hasMany('Acelle\Model\Campaign')->orderBy('campaigns.created_at', 'desc');
    }

    public function sentCampaigns()
    {
        return $this->hasMany('Acelle\Model\Campaign')->whereNull('del_date')->where('status', '=', 'done')->orderBy('created_at', 'desc');
    }
    ...
    /* this function is used to get the archived campaigns
    ...
    */
    ...
    public function getArchivedCampaigns() {
        ...
        return $this->campaigns()->where('is_auto', '=', false)->whereNotNull('archived');
    ...
    {
        return $this->campaigns()->where('is_auto', '=', false)->whereNull('del_date');
/*            ->select(['campaigns_lists_segments.created_at as creatas','contacts.created_at as creatas2'])
//            ->join('campaigns_lists_segments', 'campaigns_lists_segments.campaign_id', '=', 'campaigns.id')
//            ->join('contacts','campaigns_lists_segments.mail_list_id', '=','contacts.id')*/
    ...
    {
        MailLog::info("If you see this text in the log file, it appears that the unused code section is being used, quickly go to app/Model/Customer.php and disable it in updateCache function...");
        // cache indexes
        $index = [
            ...
            'SubscribedCount' => function(&$customer) {
                return $customer->getSubscriberCountByStatus(Subscriber::STATUS_SUBSCRIBED)+$customer->getSubscriberCountByStatus(Subscriber::STATUS_OPENER);
            },
            'OpenerCount' => function(&$customer) {
                return $customer->getSubscriberCountByStatus(Subscriber::STATUS_OPENER);
            },
    ...
    {
        try {
            $cache = json_decode(\Redis::get('customer_'.$this->id.'_cache'),true);
            MailLog::info("Getting data from redis cache for customer: ".$this->id);
            ...
            MailLog::info("Unable to get cache from redis for customer: ".$this->id." (".$this->uid.")");
            $cache = json_decode($this->cache, true);
            if (is_null($cache)) {
                return $default;
            }
     ...
     * @return void
     */
    public function addEmaillToBlacklist($email,$comment)
    {
        ...
        if (\Acelle\Library\Tool::isValidEmail($email)) {
            ...
            if (!$exist) {
                ...
                if (!empty($comment)) $blacklist->reason = $comment;
                else $blacklist->reason = 'Imported';
