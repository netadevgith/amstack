...
class Blacklist_domains extends Model
{
    ...
    {
        return self::select('blacklist_domains.*');
    }
    ...
    {
        $user = $request->user();
        $query = self::select('blacklist_domains.*');

        // Keyword
        if (!empty(trim($request->keyword))) {
            foreach (explode(' ', trim($request->keyword)) as $keyword) {
                $query = $query->where(function ($q) use ($keyword) {
                    $q->orwhere('blacklist_domains.domain', 'like', '%'.$keyword.'%');
                });
            }
        }
        ...
        if(!empty($request->customer_id)) {
            $query = $query->where('blacklist_domains.customer_id', '=', $request->customer_id);
        }
