...
class Blacklist_names extends Model
{
    ...
    protected $table = 'blacklist_names';
    ...
    {
        return self::select('blacklist_names.*');
    }
    ...
    {
        $user = $request->user();
        $query = self::select('blacklist_names.*');

        // Keyword
        if (!empty(trim($request->keyword))) {
            foreach (explode(' ', trim($request->keyword)) as $keyword) {
                $query = $query->where(function ($q) use ($keyword) {
                    $q->orwhere('blacklist_names.name', 'like', '%'.$keyword.'%');
                });
            }
        }
        ...
        if(!empty($request->customer_id)) {
            $query = $query->where('blacklist_names.customer_id', '=', $request->customer_id);
        }
