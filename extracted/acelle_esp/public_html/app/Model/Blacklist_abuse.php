...
class Blacklist_abuse extends Model
{
    ...
    protected $table = 'blacklist_abuse';
    ...
    {
        return self::select('blacklist_abuse.*');
    }
    ...
    {
        $user = $request->user();
        $query = self::select('blacklist_abuse.*');

        // Keyword
        if (!empty(trim($request->keyword))) {
            foreach (explode(' ', trim($request->keyword)) as $keyword) {
                $query = $query->where(function ($q) use ($keyword) {
                    $q->orwhere('blacklist_abuse.email', 'like', '%'.$keyword.'%');
                });
            }
        }
        ...
        if(!empty($request->customer_id)) {
            $query = $query->where('blacklist_abuse.customer_id', '=', $request->customer_id);
        }
