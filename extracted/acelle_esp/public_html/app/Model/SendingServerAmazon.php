...
class SendingServerAmazon extends SendingServer
{
    ...
    public $notificationTypes = array('Delivery','Bounce', 'Complaint');
...
//            $this->sesClient()->SetIdentityHeadersInNotificationsEnabled(array(
//                'Identity' => $awsIdentity, // extract domain from email
//                'ForwardingEnabled' => true,
//            ));
        }
        MailLog::info("before create topic");
        $topicResponse = $this->snsClient()->createTopic(array('Name' => self::SNS_TOPIC));
        MailLog::info("After create topic");
        $subscribeUrl = StringHelper::joinUrl(Setting::get('url_delivery_handler'), self::SNS_TYPE);
        // fix, we should replace {DOMAIN} with current tracking domain
        $domain = Setting::get('global_tracking');
        $subscribeUrl = str_replace('{DOMAIN}',$domain,$subscribeUrl);

        MailLog::info("before subscribe");
        // we should list all subscriptions here and unsubscribe the unecessary ones
        try {
        $resp = $this->snsClient()->listSubscriptions();
        foreach ($resp as $subscr) {
            if (is_array($subscr)) {
                foreach ($subscr as $snsitem) {
                    if (isset($snsitem['Endpoint'])) {
                        if (strpos($snsitem['Endpoint'],$domain) !== false){
                            MailLog::info("Found our domain on: ".$snsitem['Endpoint']);
                        } else {
                            MailLog::info("Amazon ses subscribe, domain is not ours: ".$snsitem['Endpoint']." cancelling subscription for it!");
                            $respo = $this->snsClient()->unsubscribe(array('SubscriptionArn'=> $snsitem['SubscriptionArn']));
                            MailLog::info("Unsubscribe response: ".print_r($respo));
                        }
                    }
                }
            }
            ...
            MailLog::error("We failed some procedures at the unsubscribe field of the amazon ses! ".$ex);
        }
        ...
        foreach ($this->notificationTypes as $type) {
            ...
            // some hack to add custom headers support
            $this->sesClient()->SetIdentityHeadersInNotificationsEnabled(array(
                'Identity' => $awsIdentity, // extract domain from email
                'NotificationType' => $type,
               'Enabled' => true,
