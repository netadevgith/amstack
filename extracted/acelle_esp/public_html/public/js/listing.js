...
function tableFilter(form, custom_url) {
    ...
    var sort_order2 = form.find("select[name='sort-order2']").val();
...
$(function () {
    var selectedValz = $.cookie("sort-order");
    if (selectedValz) {
        $("#sort-order").val(selectedValz);
        $("#sort-order").click();
    }

    var selectedVal = $.cookie("sort-order2");
    if (selectedVal) {
        $("#sort-order2").val(selectedVal);
        $("#sort-order2").click();
    }
});
...
$(document).ready(function() {
  ...
  //  $(document).ready(function(){
    ...
    $("#sort-order").on("change", function(){
        var selectionz = $(this).val();
        $.cookie("sort-order", selectionz, {expires: 365, path: '/'})
    });

        $("#sort-order2").on("change", function(){
            var selection = $(this).val();
            $.cookie("sort-order2", selection, {expires: 365, path: '/'})
        });
    //});
