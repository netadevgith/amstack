...
/*  HACK to disable DELETE typing verification       if($('.confirm-delete-form').valid()) {
    ...
    // send test mail from all domains implementation 2020.10.26
    $(document).on('click', '.send-a-test-domain-link', function(e) {
        var uid = $(this).attr("data-uid");
        $('input[name=send_test_domain_campaign_uid]').val(uid);
        $('#send_a_test_domain').modal("show");
        e.preventDefault();
    });

    $(".ajax_send_a_test_domain_form").submit(function(e) {
        var url = $(this).attr("action");
        var form = $(".ajax_send_a_test_domain_form");
    ...
    // send test mail to all servers implementation 2019.04.26
    $(document).on('click', '.send-a-test-mass-link', function(e) {
        var uid = $(this).attr("data-uid");

        $('input[name=send_test_mass_campaign_uid]').val(uid);

        $('#send_a_test_mass').modal("show");
        e.preventDefault();
    });

    $(".ajax_send_a_test_mass_form").submit(function(e) {
        var url = $(this).attr("action");
        var form = $(".ajax_send_a_test_mass_form");
    ...
    $(document).on('click', '.stop-simulation', function(e) {
        var uid = $(this).attr("data-uid");
        jQuery.ajax({
            type:"GET",
            url:"/campaigns/simulation-stop/"+uid,
            success:function(data) {
                jQuery('#start_simulation').show();
                jQuery('#stop_simulation').hide();
            }
        });
        e.preventDefault();
    });

    $(document).on('click', '.send-simulation', function(e) {
        var uid = $(this).attr("data-uid");

        $('input[name=simulation_campaign_uid]').val(uid);

        $('#simulation_test').modal("show");
        //   alert('test');
        e.preventDefault();
    });


    $(".simulation_test_form").submit(function(e) {
        var url = $(this).attr("action");
        var form = $(".simulation_test_form");
      //  var uid = $('input[name=simulation_campaign_uid]').val(uid);
        ...
        if(form.valid()) {
            ...
            $.ajax({
                ...
                {
                    $("#display_debug").show();
                    jQuery('#start_simulation').hide();
                    jQuery('#stop_simulation').show();
