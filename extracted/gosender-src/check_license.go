...
func ChangeCampaignError(uid, status string) {
// License error types
...
// 1007 Invalid license
        ...
        STATUS := fmt.Sprintf("Error: %s",status)
        ...
        updeitas := `UPDATE campaigns set status = 'error', last_error = ? WHERE uid = ?`
...
func VerifyRemotely(serverURL string, licenseFile string) (verified bool, err error) {
	form := url.Values{}
        ...
        if lic_exist == false {
	   ChangeCampaignError(campaign,"License 1001")
        }

        tmp_lic, err := ioutil.ReadFile(licenseFile)
        if err != nil {
	        ChangeCampaignError(campaign,"License 1002")
         }
	...
	request.Header.Set("Content-Type", "application/x-www-form-urlencoded")
        ...
        if err != nil {
		ChangeCampaignError(campaign,"License 1003")
        }
	...
	if err != nil {
		ChangeCampaignError(campaign,"License 1004")
		return false, err
	}
	...
	if err != nil {
                ChangeCampaignError(campaign,"License 1005")
		return false, err
	}

	err = json.Unmarshal(bytes, &res)
	if err != nil {
		ChangeCampaignError(campaign,"License 1006")
		return false, err
	}

	errMsg, ok := res["error"]
	if ok {
      	        ChangeCampaignError(campaign,"License 1007")
