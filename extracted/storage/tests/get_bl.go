...
EMAIL := os.Args[1]
if get_bl(EMAIL) {
fmt.Printf("The email: %s exist on the remote blacklists storage engine\n",EMAIL)
} else {
fmt.Printf("The email: %s does not exist on the remote blacklists storage engine\n",EMAIL)
}
}

func get_bl(record string) bool {
url := "http://116.202.114.76:8081/api/v1/blacklists/check/"+record
